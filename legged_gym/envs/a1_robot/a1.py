...
from legged_gym.envs.a1_robot import a1_robot_velocity_estimator
from legged_gym.envs.a1_robot import laikago_constants
from legged_gym.envs.a1_robot import laikago_motor
...
TOE_NAME_PATTERN = re.compile(r"\w+_foot_\w+")
IMU_NAME_PATTERN = re.compile(r"\w+imu\w+")
...
class A1(minitaur.Minitaur):
    ...
    for ee_name in FOOT_NAMES:
      self.chain_ee.append(
          pk.build_serial_chain_from_urdf(
            open(URDF_FILENAME).read(), ee_name).to(device='cpu'))
  ...
  def _BuildUrdfIds(self):
    ...
    for i in range(num_joints):
      ...
      # the urdf version used.
        ...
        print("Uknown category of joint %s" % joint_name)
  ...
  def GetFootPositionsInBaseFrame(self):
    ...
    for i, chain_ee in enumerate(self.chain_ee):
      foot_pos.append(
          chain_ee.forward_kinematics(motor_angles[i * 3:i * 3 +
            3]).get_matrix()[:, :3, 3]).cpu().numpy()
    return np.array(foot_pos)
