...
from legged_gym.envs.a1_robot import minitaur_pose_utils
...
class MotorAngleSensor(sensor.BoxSpaceSensor):
  ...
  def __init__(self,
               ...
               lower_bound: _FLOAT_OR_ARRAY = -2 * np.pi,
               upper_bound: _FLOAT_OR_ARRAY = 2 * np.pi,
    ...
    Args:
      ...
      default_pose: Default pose for observation scaling.
      scales: Observation scale.
  ...
  def _get_observation(self) -> _ARRAY:
    ...
    motor_angles = (motor_angles - self.default_pose) * self.scales
...
class MotorVelocitySensor(sensor.BoxSpaceSensor):
  ...
  def __init__(self,
               num_motors: int,
               noisy_reading: bool = True,
               lower_bound: _FLOAT_OR_ARRAY = -2000 * np.pi,
               upper_bound: _FLOAT_OR_ARRAY = 2000 * np.pi,
  ...
  def _get_observation(self) -> _ARRAY:
    if self._noisy_reading:
      motor_vels = self._robot.GetMotorVelocities()
    else:
      motor_vels = self._robot.GetTrueMotorVelocities()

    motor_vels = motor_vels * self.scales

    return motor_vels.astype(self._dtype)
...
class ProjectedGravitySensor(sensor.BoxSpaceSensor):
  """A sensor that reads motor angles from the robot."""

  def __init__(self,
               lower_bound = [-1.0] * 3,
               upper_bound = [1.0] * 3,
               name: typing.Text = "ProjectedGravity",
               dtype: typing.Type[typing.Any] = np.float64) -> None:
    """Constructs MotorVelocitySensor.
    """

    super(ProjectedGravitySensor, self).__init__(
        name=name,
        shape=(3,),
        lower_bound=lower_bound,
        upper_bound=upper_bound,
  ...
  def _get_observation(self) -> _ARRAY:
    orn = np.array(self._robot.GetTrueBaseOrientation())
    return quat_rotate_inverse(orn,  np.array([0, 0, -1]).astype(self._dtype))

class FakeCommandSensor(sensor.BoxSpaceSensor):
  """A sensor that reads motor angles from the robot."""

  def __init__(self,
               lower_bound = [-1.0] * 3,
               upper_bound = [1.0] * 3,
               name: typing.Text = "FakeCommand",
               dtype: typing.Type[typing.Any] = np.float64) -> None:
    """Constructs MotorVelocitySensor.
    """

    super(FakeCommandSensor, self).__init__(
        name=name,
        shape=(3,),
        lower_bound=lower_bound,
        upper_bound=upper_bound,
...
class BaseVelocitySensor(sensor.BoxSpaceSensor):
  ...
  def __init__(self,
               scales = [2.0, 2.0, 2.0, 0.25, 0.25, 0.25],
    ...
    """
    # Compute the default lower and upper bounds
    lower_bound = [-1000] * 3 + [-2000 * np.pi] * 3
    upper_bound = [1000] * 3 + [2000 * np.pi] * 3
  ...
  def _get_observation(self) -> _ARRAY:
    ...
    if not self._use_real_robot:
      lin_vel = self._robot.TransformAngularVelocityToLocalFrame(self._robot.GetBaseVelocity(), self._robot.GetTrueBaseOrientation())
    else:
      raise ValueError
    ang_vel = self._robot.GetTrueBaseRollPitchYawRate()

    observations = np.concatenate([lin_vel, ang_vel]) * self.scales

    return observations.astype(self._dtype)
